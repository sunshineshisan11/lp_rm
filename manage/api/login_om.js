...
router.get('/set/pair', async (ctx, res, req) => {
    ...
    try {
        console.log(request)
        var sql = `UPDATE pair SET account='${request.account}',password='${request.password}',inviteCode='${request.inviteCode}',dataError='${request.dataError}',
        ...
        city='${request.city}',remake='${request.remake}',lineCode='${request.lineCode}' 
        where id='${request.id}'`
        console.log(sql)
        const [rows] = await pool.execute(sql);
        console.log(rows)
...
router.get('/add/pair', async (ctx, res, req) => {
    ...
    try {
        console.log(request)
        var sql = `insert into pair(account, avatar,pairAccount, pairAvatar, vipCode, date, status) values('${request.account}','${request.avatar}','${request.pairAvatar}','${request.vipCode}','${tools.DFormat(new Date())}','${request.status}');`
        console.log(sql)
        const [rows] = await pool.execute(sql);
        console.log(rows)
        ctx.body = {
...
router.get('/del/pair', async (ctx, res, req) => {
    ...
    try {
        console.log(request)
        var sql = `delete from pair where id = '${request.id}'`
        console.log(sql)
        const [rows] = await pool.execute(sql);
        console.log(rows)
        ctx.body = {
...
router.get('/add/convension', async (ctx, res, req) => {
    ...
    try {
        console.log(request)
        var sql = `insert into convension(account, height, weight, xw, job, city,avatar,jj,video,name,gender,inviteCode) values('${request.account}','${request.height}','${request.weight}','${request.xw}','${request.job}','${request.city}','${request.avatar}','${request.jj}','${request.video}','${request.name}','${request.gender}','${request.inviteCode}');`
        console.log(sql)
        const [rows] = await pool.execute(sql);
        console.log(rows)
        ctx.body = {
...
router.get('/update/convension', async (ctx, res, req) => {
    ...
    try {
        console.log(request)
        var sql = `update convension set account='${request.account}',height='${request.height}',weight='${request.weight}',xw='${request.xw}',job='${request.job}',city='${request.city}',avatar='${request.avatar}',jj='${request.jj}',video='${request.video}',name='${request.name}',inviteCode='${request.inviteCode}' where id='${request.id}'`
        console.log(sql)
        const [rows] = await pool.execute(sql);
        console.log(rows)
        ctx.body = {
